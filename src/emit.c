	...
	for (uint_fast64_t i = 0; i < count; i++) {
		...
		case COMPILER_OP_CODE_SET:
			...
			if (instructions[i].regs[2].reg) { //atomotize signature
				fprintf(file_out, ".long_int = defined_sig_count; sig=new_type_sig(); PANIC_ON_FAIL(sig, CISH_ERROR_MEMORY); "
		...
		case COMPILER_OP_CODE_POP_ATOM_TYPESIGS:
			fprintf(file_out, "if(%"PRIu16" > defined_sig_count) { PANIC(CISH_ERROR_STACK_OVERFLOW); }; \n", instructions[i].regs[0].reg);
		...
		case COMPILER_OP_CODE_CALL:
			fputs("PANIC_ON_FAIL(position_count != FRAME_LIMIT, CISH_ERROR_STACK_OVERFLOW);", file_out);
		...
		case COMPILER_OP_CODE_LOAD_ALLOC:
			...
			//bounds check
			fputs("PANIC_ON_FAIL(scratch_i < scratch_heap->limit, CISH_ERROR_INDEX_OUT_OF_RANGE);", file_out);
			//mem init check
			fputs("PANIC_ON_FAIL(scratch_heap->init_stat[scratch_i], CISH_ERROR_READ_UNINIT);", file_out);
		...
		case COMPILER_OP_CODE_LOAD_ALLOC_I:
			...
			//mem init check
			fprintf(file_out, "PANIC_ON_FAIL(scratch_heap->init_stat[%"PRIu16"], CISH_ERROR_READ_UNINIT);", instructions[i].regs[2].reg);
		...
		case COMPILER_OP_CODE_LOAD_ALLOC_I_BOUND:
			...
			//bounds check
			fprintf(file_out, "PANIC_ON_FAIL(%"PRIu16" < scratch_heap->limit, CISH_ERROR_INDEX_OUT_OF_RANGE);", instructions[i].regs[2].reg);
		...
		case COMPILER_OP_CODE_FREE:
			...
			emit_reg(file_out, instructions[i].regs[0], 0);
			fputs(".heap_alloc), CISH_ERROR_MEMORY);", file_out);
			if (instructions[i].op_code == COMPILER_OP_CODE_DYNAMIC_FREE)
				fputc('}', file_out);
			break;
		case COMPILER_OP_CODE_GC_NEW_FRAME:
			fputs("PANIC_ON_FAIL(heap_frame != FRAME_LIMIT, CISH_ERROR_STACK_OVERFLOW);"
		...
		case COMPILER_OP_CODE_FLOAT_DIVIDE: {
			...
			if (op_id <= (COMPILER_OP_CODE_LONG_DIVIDE - COMPILER_OP_CODE_LONG_MORE) || instructions[i].op_code == COMPILER_OP_CODE_LONG_MODULO) {
				if (instructions[i].op_code == COMPILER_OP_CODE_LONG_DIVIDE) {
					...
					emit_reg(file_out, instructions[i].regs[1], 0);
					fputs(".long_int, CISH_ERROR_DIVIDE_BY_ZERO);", file_out);
				}
				emit_reg(file_out, instructions[i].regs[2], 0);
		...
		case COMPILER_OP_CODE_CONFIG_TYPESIG:
			if (instructions[i].regs[2].reg) {
				fputs("PANIC_ON_FAIL(sig = malloc(sizeof(machine_type_sig_t)), CISH_ERROR_MEMORY);", file_out);
		...
		case COMPILER_OP_CODE_RUNTIME_TYPECAST:
			...
			emit_reg(file_out, instructions[i].regs[0], 0);
			fprintf(file_out, ".heap_alloc->type_sig, defined_signatures[%"PRIu16"]), CISH_ERROR_UNEXPECTED_TYPE);", instructions[i].regs[2].reg);
			emit_reg(file_out, instructions[i].regs[1], 0);
			fputs(".heap_alloc = ", file_out);
			emit_reg(file_out, instructions[i].regs[0], 0);
			fputs(".heap_alloc;", file_out);
		...
		case COMPILER_OP_CODE_DYNAMIC_TYPECAST_DD:
			...
			fputs(".long_int], defined_signatures[", file_out);
			emit_reg(file_out, instructions[i].regs[2], 0);
			fputs(".long_int]), CISH_ERROR_UNEXPECTED_TYPE);", file_out);
			break;
		case COMPILER_OP_CODE_DYNAMIC_TYPECAST_DR:
			...
			fputs(".heap_alloc->type_sig : defined_signatures[", file_out);
			emit_reg(file_out, instructions[i].regs[1], 0);
			fprintf(file_out, ".long_int], defined_signatures[%"PRIu16"]), CISH_ERROR_UNEXPECTED_TYPE);", instructions[i].regs[2].reg);
			break;
